...
class FeedbackManager:
    ...
    def collect_feedback(self, job_name: str, candidate_name: str, feedback: HumanFeedback) -> None:
        ...
        if not eval_path.exists():
            raise ValueError(f"No evaluation found for {candidate_name} in job {job_name}")
    ...
    def build_insights(self, job_name: str) -> Optional[JobInsights]:
        ...
        if len(feedback_records) < 2:
            print(f"⚠️  Need at least 2 feedback records to generate insights (have {len(feedback_records)})")
            return None
        ...
        job_insights = JobInsights(
            ...
            effectiveness_metrics=self._calculate_effectiveness_metrics(feedback_records)
        )
            ...
            if candidate_dir.is_dir():
                ...
                if feedback_path.exists():
                    try:
                        ...
                        print(f"Warning: Could not load feedback for {candidate_dir.name}: {e}")
        
        return feedback_records
    
    def trigger_re_evaluation(self, job_name: str, candidate_names: Optional[List[str]] = None) -> None:
        ...
        for candidate_dir in candidates_path.iterdir():
            if candidate_dir.is_dir():
                ...
                print(f"🔄 Re-evaluating {candidate_name} with updated insights...")
                print(f"🤖 Using AI Model: {self.ai_client.model}")
                evaluation = self.ai_client.evaluate_candidate(
                    job_context, 
                    candidate, 
                    job_insights.generated_insights
                )
                ...
                print(f"✅ Re-evaluated {candidate_name} (Score: {evaluation.overall_score}/100)")
        ...
        if re_evaluated:
            ...
            try:
                ...
                if evaluations:
                    ...
                    csv_path = output_gen.generate_csv(evaluations, output_path, job_name)
                    html_path = output_gen.generate_html_report(job_context, evaluations, output_path)
    ...
    def _update_job_feedback_summary(self, job_name: str, feedback_record: FeedbackRecord) -> None:
        """Update job-level feedback summary."""
    ...
    def _save_evaluation_with_history(self, candidate_dir: Path, evaluation: Evaluation) -> None:
    ...
    def _calculate_effectiveness_metrics(self, feedback_records: List[FeedbackRecord]) -> Dict[str, float]:
