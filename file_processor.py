...
class FileProcessor:
    ...
    def process_job_intake(self, job_name: str) -> Tuple[JobFiles, List[str]]:
        ...
        if job_desc_path:
            ...
            errors.append("Job description file not found. Expected: job_description.pdf")
    ...
    def process_candidate_intake(self, job_name: str) -> Tuple[List[CandidateFiles], List[str]]:
        ...
        if unrecognized:
            ...
            if self.interactive:
                try:
                    confirm = input("\nContinue processing recognized candidates? (y/N): ").strip().lower()
                    if confirm not in ['y', 'yes']:
                        errors.append("Processing cancelled by user. Please fix unrecognized files.")
        ...
        if not candidate_files:
            ...
            errors.append("Expected naming: resume_firstname_lastname.pdf, coverletter_firstname_lastname.pdf, application_firstname_lastname.txt")
            return candidates, errors
    ...
    def organize_job_files(self, job_files: JobFiles) -> Tuple[JobContext, List[str]]:
        ...
        description, desc_error = self.extract_text_from_file(job_files.job_description_path)
        if desc_error:
            errors.append(f"Job description: {desc_error}")
            description = ""
        else:
            # Move file to job directory
            self._move_file_to_job_dir(job_files.job_description_path, job_dir, "job_description")
        
        # Extract ideal candidate (optional)
        ideal_candidate = None
        if job_files.ideal_candidate_path:
            ideal_text, ideal_error = self.extract_text_from_file(job_files.ideal_candidate_path)
            if ideal_error:
                errors.append(f"Ideal candidate: {ideal_error}")
            else:
                ideal_candidate = ideal_text
                self._move_file_to_job_dir(job_files.ideal_candidate_path, job_dir, "ideal_candidate")
        
        # Extract warning flags (optional)
        warning_flags = None
        if job_files.warning_flags_path:
            warning_text, warning_error = self.extract_text_from_file(job_files.warning_flags_path)
            if warning_error:
                errors.append(f"Warning flags: {warning_error}")
            else:
                warning_flags = warning_text
                self._move_file_to_job_dir(job_files.warning_flags_path, job_dir, "warning_flags")
    ...
    def organize_candidate_files(self, candidate_files: CandidateFiles, job_name: str) -> Tuple[Candidate, List[str]]:
        ...
        candidate_dir = self.config.get_candidate_path(job_name, candidate_files.candidate_name)
        ...
        if not candidate_files.resume_path:
            ...
            return Candidate(name=candidate_files.candidate_name, resume_text=""), errors
        
        resume_text, resume_error = self.extract_text_from_file(candidate_files.resume_path)
        if resume_error:
            errors.append(f"{candidate_files.candidate_name} resume: {resume_error}")
            resume_text = ""
        else:
            self._move_file_to_candidate_dir(candidate_files.resume_path, candidate_dir, "resume")
        
        # Extract cover letter (optional)
        cover_letter = None
        if candidate_files.cover_letter_path:
            cover_text, cover_error = self.extract_text_from_file(candidate_files.cover_letter_path)
            if cover_error:
                errors.append(f"{candidate_files.candidate_name} cover letter: {cover_error}")
            else:
                cover_letter = cover_text
                self._move_file_to_candidate_dir(candidate_files.cover_letter_path, candidate_dir, "cover_letter")
        
        # Extract application (optional)
        application = None
        if candidate_files.application_path:
            app_text, app_error = self.extract_text_from_file(candidate_files.application_path)
            if app_error:
                errors.append(f"{candidate_files.candidate_name} application: {app_error}")
            else:
                application = app_text
                self._move_file_to_candidate_dir(candidate_files.application_path, candidate_dir, "application")
    ...
    def _find_candidate_files(self) -> Dict[str, str]:
        ...
        expected_prefixes = ['resume_', 'coverletter_', 'cover_', 'cover_letter_', 'application_', 'questionnaire_']
        ...
        for file_path in intake_path.iterdir():
            if file_path.is_file():
                ...
                if not matched:
                    corrected_path = self._try_typo_correction(file_path, expected_prefixes)
                    if corrected_path:
    ...
    def _try_typo_correction(self, file_path: Path, expected_prefixes: List[str]) -> Optional[str]:
        ...
        if not any(word in filename_lower for word in ['resum', 'cover', 'coverletter', 'cover_letter', 'application', 'questionnaire', 'cv', 'applicat']):
        ...
        for prefix in expected_prefixes:
            ...
            filename_start = filename.split('_')[0].lower()  # Get first part before underscore
    ...
    def _group_files_by_candidate(self, candidate_files: Dict[str, str]) -> Dict[str, Dict[str, str]]:
        ...
        for file_path, file_type in candidate_files.items():
            ...
            type_aliases = {'resume', 'coverletter', 'cover', 'cover_letter', 'application', 'questionnaire'}
        ...
        for candidate_name, types_map in candidates.items():
            ...
            if duplicates_detected.get(candidate_name):
                ...
                print(f"🔁 Merged duplicates for {candidate_name}: {merged_types} (kept newest by modified time)")
    ...
    def _move_file_to_job_dir(self, source_path: str, job_dir: str, base_name: str) -> None:
    ...
    def _move_file_to_candidate_dir(self, source_path: str, candidate_dir: str, base_name: str) -> None:
    ...
    def _check_unrecognized_files(self) -> List[Tuple[str, Optional[str]]]:
        ...
        for file_path in intake_path.iterdir():
            ...
            if any(word in filename_lower for word in ['resum', 'cv', 'cover', 'application', 'questionnaire']):
                ...
                elif 'application' in filename_lower or 'questionnaire' in filename_lower:
