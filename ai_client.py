...
class AIClient:
    ...
    def evaluate_candidate(
        ...
        try:
            response = self._make_request([
                {"role": "system", "content": "You are an expert HR professional evaluating job candidates. Provide thorough, fair, and structured evaluations."},
            ...
            return Evaluation(
                ...
                recommendation=RecommendationType(evaluation_data.get("recommendation", "NO")),
                strengths=evaluation_data.get("strengths", []),
                concerns=evaluation_data.get("concerns", []),
                interview_priority=InterviewPriority(evaluation_data.get("interview_priority", "LOW")),
    ...
    def generate_insights(
        ...
        try:
            response = self._make_request([
                {"role": "system", "content": "You are an AI learning system that generates insights from human feedback to improve candidate evaluations."},
                {"role": "user", "content": prompt}
            ])
  ...
  "recommendation_logic": "refined logic for recommendation categories"
    ...
    def _parse_evaluation_response(self, response_content: str) -> Dict:
        ...
        try:
            ...
            if validated_data["recommendation"] not in [r.value for r in RecommendationType]:
                validated_data["recommendation"] = "NO"
            
            if validated_data["interview_priority"] not in [p.value for p in InterviewPriority]:
