...
from models import JobSetupResult, ProcessingResult, DisplayResult, Evaluation, JobInsights, RecommendationType, CandidateFiles
...
class CandidateReviewer:
    ...
    def __init__(self, config: Config):
        ...
        if config.openai_api_key:
            ...
            connection_tester = OpenAIConnectionTester(config.openai_api_key, config.preferred_model)
        ...
        self.feedback_manager = FeedbackManager(config, self.ai_client) if self.ai_client else None
    ...
    def setup_job_with_paths(self, job_name: str, job_description_path: str = None, 
                           ideal_candidate_path: str = None, warning_flags_path: str = None,
                           allow_update: bool = False) -> JobSetupResult:
        ...
        try:
            ...
            if not job_description_path:
                return JobSetupResult(
                    ...
                    errors=["Use -j /path/to/job_description.pdf or place job_description.pdf in intake/"]
                )
    ...
    def process_single_candidate(self, job_name: str, candidate_name: str, 
                               resume_path: str, cover_letter_path: str = None, 
                               application_path: str = None) -> ProcessingResult:
        ...
        try:
            ...
            candidate, cand_errors = self.file_processor.organize_candidate_files(candidate_files, job_name)
            ...
            csv_path = self.output_generator.generate_csv(all_evaluations, output_path, job_name)
            html_path = self.output_generator.generate_html_report(job_context, all_evaluations, output_path)
            
            print(f"✅ Processed: {candidate.name} (Score: {evaluation.overall_score}/100)")
    ...
    def process_candidates(self, job_name: str) -> ProcessingResult:
        ...
        try:
            ...
            candidate_files_list, file_errors = self.file_processor.process_candidate_intake(job_name)
            ...
            if not candidate_files_list and not file_errors:
                return ProcessingResult(
                    ...
                    errors=["Place candidate files in intake directory with naming: resume_firstname_lastname.pdf"]
                )
                ...
                try:
                    ...
                    if cand_errors:
                        ...
                        return ProcessingResult(
                            ...
                            errors=all_errors,
                            message=(f"Stopped at candidate '{candidate_files.candidate_name}' due to missing/invalid files. "
                    ...
                    while attempt < max_retries:
                        ...
                        if evaluation and evaluation.overall_score == 0:
                            for c in (evaluation.concerns or []):
                                msg = str(c).lower()
                                if 'api error' in msg or 'connection error' in msg or 'rate limit' in msg:
                                    retryable = True
                                    break
                        if not retryable:
                            break
                        ...
                        if attempt < max_retries:
                            ...
                            print(f"⏳ Transient API error for {candidate.name}. Retrying in {wait}s (attempt {attempt}/{max_retries-1})...")
                            time.sleep(wait)
                    
                    # If still failed due to API error, mark as failed and skip saving
                    if evaluation and evaluation.overall_score == 0:
                        has_api_issue = any('api error' in str(c).lower() or 'connection error' in str(c).lower() for c in (evaluation.concerns or []))
                    else:
                        has_api_issue = False
                    
                    if has_api_issue:
                        all_errors.append(f"{candidate_files.candidate_name}: Transient API error - not processed. Please retry.")
                        failed_candidates.append(candidate_files.candidate_name)
                        continue
    ...
    def provide_feedback(self, job_name: str, candidate_name: str, feedback_text: Optional[str] = None) -> None:
        ...
        print(f"   Strengths: {', '.join(evaluation.strengths[:2])}{'...' if len(evaluation.strengths) > 2 else ''}")
        print(f"   Concerns: {', '.join(evaluation.concerns[:2])}{'...' if len(evaluation.concerns) > 2 else ''}")
        ...
        if feedback_text:
            ...
            if any(word in feedback_lower for word in ["poor", "bad", "terrible", "awful", "reject", "no"]):
                human_recommendation = RecommendationType.NO
                human_score = 40  # Low score for negative feedback
            elif any(word in feedback_lower for word in ["excellent", "great", "perfect", "hire", "strong"]):
            ...
            if "english" in feedback_lower or "grammar" in feedback_lower:
                corrections["concerns"] = f"{', '.join(evaluation.concerns)}, Poor written English with grammatical mistakes"
            ...
            human_score = None
            score_input = input(f"\nOptional: Provide a score 0-100 (or press Enter to skip): ").strip()
            if score_input:
                try:
            ...
            print("Available fields: overall_score, strengths, concerns, detailed_notes")
    ...
    def show_insights(self, job_name: str) -> None:
        ...
        if not insights_path.exists():
            ...
            print("Insights are generated after collecting feedback on candidate evaluations.")
            return
        ...
        try:
            ...
            print(f"Last updated: {insights.last_updated.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def re_evaluate_candidates(self, job_name: str, candidate_names: Optional[List[str]] = None) -> None:
    ...
    def open_reports(self, job_name: str) -> None:
        ...
        try:
            ...
            if job_context and evaluations:
                latest_html_str = self.output_generator.generate_html_report(job_context, evaluations, output_path)
    ...
    def list_reports(self, job_name: str) -> None:
        ...
        if csv_files:
            print(f"\n📈 CSV Reports:")
            for csv_file in sorted(csv_files, key=lambda f: f.stat().st_mtime, reverse=True):
        ...
        if html_files:
            print(f"\n🌐 HTML Reports:")
            for html_file in sorted(html_files, key=lambda f: f.stat().st_mtime, reverse=True):
    ...
    def _load_job_context(self, job_name: str):
        ...
        if ideal_file.exists():
            ideal_candidate, _ = self.file_processor.extract_text_from_file(str(ideal_file))
        
        warning_flags = None
        if warning_file.exists():
            warning_flags, _ = self.file_processor.extract_text_from_file(str(warning_file))
    ...
    def _cleanup_intake_files(self, candidate_files: CandidateFiles) -> None:
        ...
        try:
            ...
            if candidate_files.resume_path and candidate_files.resume_path.startswith(self.config.intake_path):
                files_to_remove.append(candidate_files.resume_path)
            
            if candidate_files.cover_letter_path and candidate_files.cover_letter_path.startswith(self.config.intake_path):
                files_to_remove.append(candidate_files.cover_letter_path)
            
            if candidate_files.application_path and candidate_files.application_path.startswith(self.config.intake_path):
    ...
    def clean_intake(self) -> None:
        ...
        for file_path in intake_path.iterdir():
            if file_path.is_file() and file_path.name not in job_files:
                ...
                if any(file_path.name.startswith(prefix) for prefix in ['resume_', 'coverletter_', 'application_']):
                    # Only include if candidate has been processed (exists in candidates directory)
                    candidate_name = self._extract_candidate_name_from_filename(file_path.name)
                    if candidate_name and self._candidate_exists_in_system(candidate_name):
    ...
    def reset_candidates(self, job_name: str) -> None:
        ...
        try:
            confirm = input(f"\nReset candidates for job '{job_name}'? (y/N): ").strip().lower()
            if confirm in ['y', 'yes']:
                removed = 0
                for item_type, item_path in items_to_remove:
                    try:
...
def cli(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        click.echo("  python3 candidate_reviewer.py provide-feedback 'job_name' 'candidate_name' 'feedback'")
        click.echo("  python3 candidate_reviewer.py re-evaluate 'job_name'")
        click.echo("\n🔧 Utility Commands:")
        click.echo("  python3 candidate_reviewer.py clean-intake              # Clean up processed files")
        click.echo("  python3 candidate_reviewer.py open-reports 'job_name'   # Open HTML report")
        click.echo("  python3 candidate_reviewer.py list-jobs                 # Show all jobs")
...
@click.option('--update/--no-update', default=False, help='Update an existing job setup')
@click.pass_context
def setup_job(ctx, job_name: str, job_description: str = None, ideal_candidate: str = None, warning_flags: str = None, update: bool = False):
    ...
    if update and job_name and job_name.isdigit() and not (job_description or ideal_candidate or warning_flags):
    ...
    if job_description or ideal_candidate or warning_flags:
        ...
        result = reviewer.setup_job_with_paths(job_name, job_description, ideal_candidate, warning_flags, allow_update=update)
    else:
    ...
    if result.success:
        ...
        if result.job_context:
            click.echo(f"   Job description loaded: {len(result.job_context.description)} characters")
        ...
        click.echo("   Direct paths: setup-job 'job_name' -j /path/to/job.pdf [--update]")
        sys.exit(1)
...
@click.option('--candidate-name', '-n', help='Candidate name (if not using intake naming)')
@click.pass_context
def process_candidates(ctx, job_name: str, resume: str = None, cover_letter: str = None, 
                      application: str = None, candidate_name: str = None):
    ...
    if resume:
        ...
        if not candidate_name:
            ...
            click.echo("💡 Usage: process-candidates 'job_name' -r /path/to/resume.pdf -n 'John Doe'")
            sys.exit(1)
        
        result = reviewer.process_single_candidate(job_name, candidate_name, resume, cover_letter, application)
    else:
        # Use intake directory
    ...
    if result.success:
        ...
        if result.processed_candidates:
            click.echo(f"   Successfully processed: {', '.join(result.processed_candidates)}")
        ...
        click.echo("   Direct paths: process-candidates 'job_name' -r /path/to/resume.pdf -n 'John Doe'")
        sys.exit(1)
...
@click.pass_context
def provide_feedback(ctx, job_name: str, candidate_name: str, feedback_text: str = None):
    """Provide feedback on a candidate evaluation.
...
@cli.command()
@click.argument('job_name')
@click.option('--candidates', '-c', multiple=True, help='Specific candidates to re-evaluate')
@click.pass_context
