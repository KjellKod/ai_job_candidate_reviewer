...
class OutputGenerator:
    ...
    def generate_csv(self, evaluations: List[Evaluation], output_path: str, job_name: str) -> str:
    ...
    def display_terminal_ranking(self, evaluations: List[Evaluation], job_name: str) -> None:
        ...
        for rank, evaluation in enumerate(sorted_evaluations, 1):
            ...
            print(f"   Score: {evaluation.overall_score}/100 | {rec_color}{evaluation.recommendation.value}\033[0m | {priority_icon} {evaluation.interview_priority.value}")
            
            if evaluation.strengths:
                print(f"   ✅ Strengths: {', '.join(evaluation.strengths[:3])}{'...' if len(evaluation.strengths) > 3 else ''}")
            
            if evaluation.concerns:
                print(f"   ⚠️  Concerns: {', '.join(evaluation.concerns[:2])}{'...' if len(evaluation.concerns) > 2 else ''}")
    ...
    def generate_html_report(self, job_context: JobContext, evaluations: List[Evaluation], output_path: str) -> str:
            ...
            if candidate_dir.is_dir():
                ...
                if eval_path.exists():
                    try:
                        ...
                        print(f"Warning: Could not load evaluation for {candidate_dir.name}: {e}")
    ...
    def generate_summary_stats(self, evaluations: List[Evaluation]) -> Dict:
        ...
        return {
            ...
            'strong_candidates': len([e for e in evaluations if e.recommendation in [RecommendationType.STRONG_YES, RecommendationType.YES]]),
            'high_priority_interviews': len([e for e in evaluations if e.interview_priority == InterviewPriority.HIGH])
        }
    ...
    def _build_html_content(self, job_context: JobContext, evaluations: List[Evaluation]) -> str:
