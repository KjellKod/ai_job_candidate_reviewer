...
class Config:
    ...
    def _setup_paths(self) -> None:
        ...
        for path in [self.intake_path, self.jobs_path, self.candidates_path, self.output_path]:
    ...
    def validate_required_settings(self) -> ValidationResult:
        ...
        if not self.openai_api_key:
            errors.append("OPENAI_API_KEY is required. Please add it to your .env file.")
        elif not self.openai_api_key.startswith(('sk-', 'sk-proj-')):
            warnings.append("OPENAI_API_KEY format looks unusual. Expected to start with 'sk-' or 'sk-proj-'.")
        
        # Validate file size limit
        if self.max_file_size_mb <= 0:
            errors.append("MAX_FILE_SIZE_MB must be greater than 0.")
        elif self.max_file_size_mb > 10:
            warnings.append(f"MAX_FILE_SIZE_MB is set to {self.max_file_size_mb}MB. Large files may cause memory issues.")
